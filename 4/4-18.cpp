...
public:
    ...
    std::vector<std::vector<std::pair<std::size_t, std::size_t>>> DijkstraPQ(std::size_t s) {
        ...
        dists[s] = T{0};
        std::vector<std::vector<std::pair<std::size_t, std::size_t>>> prev (n);
        prev[s] = {{s, 0}};
        ...
        while (!Q.empty()) {
            ...
            for (const auto& [v, w_] : adj[u]) {
                if (dists[v] > dists[u] + w_) {
                    dists[v] = dists[u] + w_;
                    Q.emplace(dists[v], v);
                    prev[v] = {u, prev[u].back().second + 1};
                } else if (dist[v] == dists[u] + w_) {
                    if (prev[v].back().second >= prev[u].back().second) {
                        prev[v] = {u, prev[u].back().second + 1};
                    }
                }
            }

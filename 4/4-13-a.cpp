...
public:
    ...
    bool existsFeasibleRoute(std::size_t src, std::size_t dst, const T& capacity) {
        std::vector<Edge> removed_edges;
        for (std::size_t i = 0; i < n; ++i) {
            for (const auto& [j, w_] : adj[i]) {
                if (w_ > capacity) {
                    removed_edges.emplace_back(i, j, w_);
                }
            }
        }
        for (const auto& [i, j, w_] : removed_edges) {
            removeEdge(i, j, w_);
        }
        ...
        while (!to_visit.empty()) {
            ...
            to_visit.pop_front();
            visited[curr] = true;
            for (const auto& [next, w_] : adj[curr]) {
                if (!visited[next]) {
